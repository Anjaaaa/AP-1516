\subsection{Bestimmung der Mittleren Reichweite von $\alpha$-Strahlung} \label{sec:reichweite}\todo[color=red]{Wenn R die Reichweite ist, sollte dann die Einheit von b nicht cm sein?}
\todo[color=green]{nein, da y die Pulse sind und b die gleiche einheit hat wie y}
\todo[color=red]{Lach mich jetzt nicht aus, aber was ist denn m und b überhaupt?}
\todo[color=green]{die haben keine bedeutung außer steigung und y-achsenabschnitt}\todo[color=red]{Warum ist R überhaupt die Steigung an dem Punkt und nicht die Steigung am ersten Punkt oder so?}
\todo[color=green]{es ist nicht die steigung in einem punkt, sondern die steigung des linearen anteils}
...
\todo[inline, color = yellow]{irgendwie verunsichern mich deine fragen. ist mein text so missverständlich geschrieben? eigentlich ist das nämlich nicht so kompliziert, aber ich weiß nicht, wie ich es ausdrücken soll.}
...
\end{align}
Wobei $\mu$ dem Erwartungswert und $\sigma$ der Varianz der Verteilung entspricht. In diesem Falle ist\todo[color=red]{Warum diese Einheiten? Das sind doch Anzahlen von Pulsen.}
\todo[color=green]{stimmt du hast recht, danke :)}
\begin{align}
