...
\begin{align*}
	...
	p_0 &= \SI{1.01325}{\bar} \\
\end{align*}
Die errechneten Werte für den Koeffizienten $a_{\text{Luft}}$ von Luft sind in Tabelle~\ref{tab:BrechungsindexLuft} zu finden und die für Kohlenstoffdioxid  $a{\text{CO$_2$}$ in Tabelle~\ref{tab:BrechungsindexCO2}.
Die Mittelwerte sind:
\begin{align}
	a_\text{Luft} &=\input{build/a_Luft_korrigiert.tex} \\
	a_\text{CO$_2$} &= \input{build/a_CO2_korrigiert.tex}
\end{align}

Daraus ergeben sich die Brechungsindizes und ihrer Fehler:
\begin{align}
	n_\text{Luft} &=\input{build/Brechungsindex_Luft_korrigiert.tex} \\
	n_\text{CO$_2$} &= \input{build/Brechungsindex_CO2_korrigiert.tex}
\end{align}
...
\begin{table}[h!]
	...
	\begin{tabular}{ccc}
		...
		\hline
		\input{build/Tabelle4_korrigiert.tex}
	\end{tabular}
...
\begin{table}[h!]
	...
	\begin{tabular}{cccc}
		Druckunterschied / \si{\bar} & Impulse & $a_{\text{CO$_2$}}$ / \si{\per\meter}\\
		\hline
		\input{build/Tabelle6_korrigiert.tex}
	\end{tabular}
